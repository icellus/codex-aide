...
- 小任务不要被重流程拖慢
- 大任务需要时可以升级到 plan / QC / submit
- 主会话尽量只做 intake、routing、governance、integration
- 执行工作尽量交给子代理
- 热状态 JSON 化，减少跨回合和跨 session 的上下文成本

换句话说，它是一个“默认 lightweight，必要时升级”的 starter，而不是一个“默认 long-running”的 starter。
...
- 任务类别默认模式
- 什么时候从 lightweight 升级
- 什么时候启用 `prd` / `architect` / `plan` / `tester` / `coder` / `/qc` / `/submit`
- `environment setup` 的默认选择
...
- 当前仓库有哪些验证能力
- 哪些命令适合 smoke
- 哪些命令用于 lint、typecheck、build
- 哪些命令用于 unit、integration、e2e
- 哪些命令昂贵
- 哪些命令依赖服务
...
任务级功能验证 owner 应该是 `tester`。

### 4.5.1 `validation-handoff`

当任务不是纯粹的微小改动时，`tester` 应提供一个明确的任务级验证 handoff。

这个 handoff 可以是：

- tester 报告里的固定段落
- 或 `.codex/templates/validation-handoff.md` 生成的单独文档
...
如果这四点都说不清，后续的 `coder` 和 `/qc` 很容易在“是否已经验证到位”上产生分歧。
...
1. `/Aide` 识别任务是 `lightweight`
2. 必要时拉起 `coder`
3. 运行必要的 sanity checks
4. 完成
...
1. `/Aide` 或 `conduct` 判断需要 `long-running`
2. 创建或更新 `PROGRESS.md`
3. 分 checkpoint 推进
4. 必要时追加 `/qc` 或 `/submit`
...
1. `/Aide`
2. `conduct` 判断为 `long-running`
3. 使用 `PROGRESS.md`
4. 必要时加 `/qc`
5. 测试完成或 `/qc` 通过后统一进入 `/submit`
...
- 哪类 feature 默认走 `lightweight` 还是 `standard`
- 什么情况下启用 `/qc`
- 什么时候启用 `environment setup`
...
- smoke 用什么命令
- 哪个命令属于 unit / integration / e2e
- 哪些命令昂贵

如果你要改的是“某个任务应如何验收”，优先不该改这里，而应该改 `tester` 的职责说明或实际测试策略。

### 想改 `/Aide`、`/qc`、`/submit` 的行为协议
...
“一套默认 lightweight、按需升级、把技能层和运行时层分开，并尽量压缩热上下文成本的项目本地 Codex 工作流骨架。”
