...
两条线共享 `/Aide` 这个“团队秘书兼人事主管”角色，但执行角色不同、沉淀位置不同、后续动作也不同。
`/Aide` 还要负责每个任务的 team staffing：先从最小团队开始，只有在确有价值时再加角色，任务收窄后再把多余角色收回去。

除此之外，`/Aide` 还直接承担一类不进入执行线的建议型、分析型工作：
...
这类回合的交付物是结论，不是持久产物。
如果分析回合本身是 read-heavy，默认先用短生命周期、只读的 `repo_explorer` 子代理做仓库阅读，再由 `/Aide` 对用户收口回复。
...
- `/Aide` 负责首轮接应、分派、治理
- `conduct` 负责 delivery routing、环境判断与 `environment setup`
...
它不应该替执行角色做业务。它是管理者，不是默认执行者，也不是主力深度排查者。
...
- 直接回答问题，而不是为了分工强行下派
- 只读取当前回答需要的最小上下文
- 如果分析任务 read-heavy，默认把阅读工作交给短生命周期的 `repo_explorer`，再由 `/Aide` 汇总并对用户收口
- 默认不写热状态或历史登记
...
这里的 staffing 规则是：先启用能安全完成任务的最小团队。
新仓库、冷启动、上下文过期，都不代表所有角色都该一起上线。
如果 owner 已经很清楚，就先交给一个明确执行角色；只有在验证、审计或交付要求真正出现时，才继续加人。

如果 owner 已经很清楚，就应尽快分派；不要为了显得充分而先把实现细节深读一轮。
更深的实现阅读应该由真正执行的角色完成。
新 task chain 在可以分派时应优先使用真实子代理，减少主线程上下文污染。
