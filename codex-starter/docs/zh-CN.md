...
- discussion：问答、分析、方案比较、路线建议，这类没有持久产物要求的任务默认由 `/Aide` 直接处理
...
在交付线之外，还有一类常见回合需要单独理解：

- 如果用户只是提问、讨论方案、比较选项、确认取舍，而当前输出只是结论或建议，那么 `/Aide` 直接负责
- 这类回合默认不写持久状态，不因为轻量问答触发全量扫描
- 只有当目标变成文档、结构化内容、代码改动、验证或受控交付时，才升级进 product 线或 coding 线
