...
async function main() {
  ...
  try {
    ...
    for (const item of blocked) {
      const missingImplementationBrief = /Implementation Brief/i.test(String(item.note || ""));
      ...
      pushReminder(
        ...
        phase === "product_assistant"
          ...
          ? `- Blocked handoff${item.taskId ? ` for ${item.taskId}` : ""}: Implementation Brief is missing or unreadable. Stop tester, qc, and submit continuation; technical_manager should refresh the brief and gather user clarification via Aide if needed.`
