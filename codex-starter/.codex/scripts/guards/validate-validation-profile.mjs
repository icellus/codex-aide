...
function profilePath(projectDir) {
  return path.join(projectDir, ".codex", "policies", "validation-profile.json");
}
...
async function main() {
  ...
  try {
    ...
    if (!fs.existsSync(filePath)) {
      process.stdout.write(
        JSON.stringify({
          ...
          message: ".codex/policies/validation-profile.json not found"
        }) + "\n"
      );
      logger.finalize({
        status: "blocked",
    ...
    process.stdout.write(
      JSON.stringify({
        ok: true,
        file: ".codex/policies/validation-profile.json",
