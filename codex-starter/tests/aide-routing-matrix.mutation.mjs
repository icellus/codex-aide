...
const defaultRootDir = path.resolve(path.dirname(testFilePath), "..");
const contractScriptPath = path.join("tests", "aide-routing-matrix.contract.mjs");
...
function withRepoCopy(sourceRoot, prefix, run) {
  ...
  fs.cpSync(sourceRoot, tempRepo, { recursive: true });
...
if (process.argv[1] && path.resolve(process.argv[1]) === testFilePath) {
