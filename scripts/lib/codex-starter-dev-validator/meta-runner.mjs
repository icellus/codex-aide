...
function normalizeAliasMap(registry, errors) {
  ...
  if (!registry.path_aliases || typeof registry.path_aliases !== "object" || Array.isArray(registry.path_aliases)) {
    errors.push("standards/codex-starter-test-registry.json: path_aliases must be an object when provided");
    return {};
  }
...
const defaultHookRootScenario = path.join("fixtures", "codex-starter-dev", "hook-root-pass");
  ...
  "governance-target-shape": createExecutor({
    ...
    runProof: ({ repoRoot, check }) =>
      validateGovernanceTarget({
        projectDir: path.join(repoRoot, check.proof_fixture_root, check.proof_project_path || "codex-starter"),
...
function normalizeChangedFiles(changedFiles) {
  ...
  if (normalized.some((filePath) => filePath.startsWith("scripts/lib/codex-starter-dev-validator/"))) {
    normalized.push("scripts/validate-codex-starter-dev.mjs");
  }
