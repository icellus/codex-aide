...
`Aide` role definition and triage semantics are owned by `.codex/aide/skills/aide/SKILL.md`.
`.codex/aide/state/task-context.json` records current route, lifecycle status, and checkpoints.
...
- if `.codex/aide/state/task-context.json` marks `sticky_owner=technical_manager`, same-task source-code follow-up should stay on the technical-delivery line by default even when phrased as Q&A.
...
- on first contact, if `.codex/aide/state/repo-context.json` is missing, or `.codex/aide/policies/validation-profile.json` is missing or still `not-set`, `Aide` runs an independent `technical_manager` repository scan and waits before normal routing continues.
- if the user indicates the repository was initialized, reinitialized, replaced, or reset, `Aide` reruns that scan and replaces `.codex/aide/state/repo-context.json` plus `.codex/aide/policies/validation-profile.json`.
...
- `.codex/aide/policies/validation-profile.json` stays the single repository validation-baseline structure.
- the first repository scan writes `.codex/aide/policies/validation-profile.json` directly and moves `status` from `not-set` to `draft`.
- `tester` completes task-level validation against repository reality, reports unfinished checks with reasons, and sends baseline refresh feedback to `technical_manager` only for reusable repository-level gaps in `.codex/aide/policies/validation-profile.json`.
- `technical_manager` evaluates tester baseline refresh feedback and decides whether a refresh proposal should be returned to `Aide` against the current `.codex/aide/policies/validation-profile.json` structure.
- `Aide` reviews `technical_manager` refresh proposals and writes approved repository-baseline updates to `.codex/aide/policies/validation-profile.json`.
...
- `.codex/aide/state/task-context.json` is the single hot-task lifecycle record.
- raw transcript `task_started/task_complete` events are turn-scoped host events, not the hot-task lifecycle truth.
- `.codex/aide/logs/task-lifecycle/*.jsonl` records normalized per-turn lifecycle sync decisions; it complements but does not replace `.codex/aide/state/task-context.json`.
...
- if the hot task could not be correctly recovered from `.codex/aide/state/task-context.json` and `.codex/aide/progress/**` without the current turn, do not treat that turn as explanation-only.
- when a main-thread routed turn records `.codex/aide/state/pending-task-turn-result.json` with `task_update.sync=true`, the Stop hook should sync that turn into the hot task state before any interruption fallback.
...
- `.codex/aide/state/task-context.json` may keep a small bounded `recent_tasks` parking list for explicitly retired hot tasks; this is not a full historical registry.
- `node .codex/aide/scripts/context/task-reconcile.mjs` is the startup helper for interrupted hot-task review. It may suggest resume or settle, but must not auto-complete tasks.
...
1. `Aide`: on first contact if `.codex/aide/state/repo-context.json` is missing, or `.codex/aide/policies/validation-profile.json` is missing or still `not-set`, or if the user indicates repository initialization/reset semantics, launch the independent repository scan and wait.
2. `technical_manager`: run the low-context repository scan and return `status` plus `repo_context` and `validation_profile`.
3. `Aide`: treat the scan as complete only when `status=complete` and both artifacts exist, then immediately write `.codex/aide/state/repo-context.json` through `node .codex/aide/scripts/context/repo-context.mjs` and `.codex/aide/policies/validation-profile.json`.
...
- enable `long-running` mode and `.codex/aide/progress/active/<task-slug>/current.md` when work is multi-step, cross-session, blocked, or release-shaped
...
- when `completed` or `cancelled` is emitted, archive the task record to `.codex/aide/progress/archive/<task-slug>/...` after the final sync
...
- `.codex/aide/state/task-context.json`: hot task state, including route evidence (`activated_roles`, `completed_roles`, `subagent_roles`) plus current chain artifacts (`prd_path`, `architecture_path`, `implementation_brief_path`)
- `.codex/aide/state/pending-task-turn-result.json`: transient main-thread routed-turn payload consumed by the Stop hook
- `.codex/aide/state/pending-governance-result.json`: transient `Aide` governance payload consumed by governance ingest on Stop
- `.codex/aide/logs/task-lifecycle/*.jsonl`: normalized per-turn task lifecycle sync log
- `.codex/aide/state/governance-context.json`: active governance items maintained by `Aide`
- `.codex/aide/state/submit-preferences.json`: repo-local submit preferences
- `.codex/aide/state/*.demo.json`: versioned structure examples for runtime state files
- `.codex/aide/policies/delivery-policy.json`: commit, push, notification, CI, release, and fallback policy
- `.codex/aide/state/repo-context.json`: repository initialization snapshot and cached repo facts
- `.codex/aide/policies/validation-profile.json`: validation commands and constraints
...
- `.codex/aide/context/project-profile.md`: human-readable repository summary and durable notes for people
