...
function taskProgressDirectory(projectDir, taskId, deliveryMode, status = "active") {
  ...
  return projectDir
    ? path.join(projectDir, ".codex", "aide", "progress", lane, normalizedTaskId)
    : path.join(".codex", "aide", "progress", lane, normalizedTaskId);
}
...
function taskContextPath(projectDir) {
  return path.join(projectDir, ".codex", "aide", "state", "task-context.json");
}
