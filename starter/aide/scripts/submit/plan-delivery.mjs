...
function readDeliveryPolicy(projectDir) {
  const filePath = path.join(projectDir, ".codex", "aide", "policies", "delivery-policy.json");
...
function isRuntimeLocalPath(filePath) {
  ...
  if (normalized.startsWith(".codex/aide/logs/")) {
    return true;
  }
  if (normalized.startsWith(".codex/aide/artifacts/delivery/") || normalized === ".codex/aide/artifacts/delivery/") {
    return true;
  }
  if (normalized.startsWith(".codex/aide/state/") && !normalized.endsWith(".demo.json")) {
    return true;
  }
  return false;
}
