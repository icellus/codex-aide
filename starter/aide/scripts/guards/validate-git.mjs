...
function decisionForInvocation(invocation, projectDir) {
  ...
  if (invocation.subcommand === "commit" || invocation.subcommand === "push") {
    return permissionResponse(
      `Direct git ${invocation.subcommand} is not allowed. Use submit via node .codex/aide/scripts/submit/execute-delivery.mjs instead.`
    );
  }
...
function commandDecision(command, projectDir) {
  ...
  if (commandContainsGovernedGitDelivery(command)) {
    return permissionResponse("Direct git commit/push is not allowed. Use submit via node .codex/aide/scripts/submit/execute-delivery.mjs instead.");
  }
