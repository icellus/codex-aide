...
2. `.codex/aide/state/repo-context.json` if present
3. `.codex/aide/state/governance-context.json` when governance judgment matters
4. `.codex/aide/policies/routing-policy.md`
5. `.codex/aide/policies/aide-governance-policy.md` when governance judgment matters
6. `.codex/aide/policies/validation-profile.json`
7. `.codex/aide/context/project-profile.md` when repo facts or human summary are needed
8. the user's goal
...
- use `node .codex/aide/scripts/context/task-overview.mjs` at `Aide` startup or when user asks for status/history
- use `node .codex/aide/scripts/context/task-reconcile.mjs` at startup or resume to surface interrupted-task follow-up guidance without auto-settling the task.
- use `node .codex/aide/scripts/context/task-progress-sync.mjs` as a read-only reminder when a long-running hot task may have drifted from `.codex/aide/progress/**`.
- use `node .codex/aide/scripts/context/task-state.mjs` for routed-task lifecycle changes in `.codex/aide/state/task-context.json`.
- only the main agent updates `.codex/aide/state/*.json`, `.codex/aide/context/project-profile.md`, `PROGRESS.md`, or `.codex/aide/policies/validation-profile.json`
- run `node .codex/aide/scripts/guards/validate-validation-profile.mjs` before writing `.codex/aide/policies/validation-profile.json` updates
- use `node .codex/aide/scripts/governance/writeback.mjs` for accepted generic governance auto-fix attempts
- write approved `.codex/aide/policies/validation-profile.json` updates from `technical_manager` refresh proposals that stay within the current file structure
...
- on first contact, if `.codex/aide/state/repo-context.json` is missing, or `.codex/aide/policies/validation-profile.json` is missing or still `not-set`, launch an independent `technical_manager` repository scan before normal routing continues
...
- write returned `repo_context` through `node .codex/aide/scripts/context/repo-context.mjs` and returned `validation_profile` to `.codex/aide/policies/validation-profile.json` immediately
...
Maintain `.codex/aide/state/task-context.json` with:
...
Treat `.codex/aide/state/*.demo.json` as structure examples only.
Use `.codex/aide/state/*.json` as runtime state and `.codex/aide/context/project-profile.md` as repo summary context.
...
- when the task enters routed delivery, changes checkpoint/owner in a durable way, becomes blocked, or is settled, update `.codex/aide/state/task-context.json` through `node .codex/aide/scripts/context/task-state.mjs`.
...
- before closing an `Aide`-only same-task reply, ask whether the hot task could still be recovered correctly from `.codex/aide/state/task-context.json` and `.codex/aide/progress/**` without this turn. If not, persist the update or route through `technical_manager`.

Maintain `.codex/aide/state/repo-context.json` with:
...
Maintain `.codex/aide/state/governance-context.json` with:
...
Maintain `.codex/aide/policies/validation-profile.json` as repository baseline only.
...
Validate approved updates with `.codex/aide/scripts/guards/validate-validation-profile.mjs` before writing them.
...
- governance rules are owned by `.codex/aide/policies/aide-governance-policy.md`
...
- treat `.codex/aide/policies/validation-profile.json` as a special-flow target; do not send it through generic governance writeback

## Governance Structured Result

When a turn opens, updates, asks about, or auto-fixes a governance item, record this payload through `node .codex/aide/scripts/governance/record-governance-result.mjs` before the final user-facing reply.
...
Maintain `.codex/aide/context/project-profile.md` as human-readable repo summary only.
Keep task routing, task status, and submit preference state in `.codex/aide/state/*.json`.
